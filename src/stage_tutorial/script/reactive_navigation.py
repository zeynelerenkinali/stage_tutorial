...
class ReactiveNavigation():
    def __init__(self):
        ...
        #one call back for each function
        #loop for each robot
    ...
    def publisher_robot(self,robot,lase_msg):
        ...
        if robot == "robot_0":
            rospy.Subscriber(_laser_topic, LaserScan, self.laser_cb_0, queue_size=1)
        elif robot == "robot_1":
            rospy.Subscriber(_laser_topic, LaserScan, self.laser_cb_1, queue_size=1)
        elif robot == "robot_2":
            rospy.Subscriber(_laser_topic, LaserScan, self.laser_cb_2, queue_size=1)
        elif robot == "robot_3":
            rospy.Subscriber(_laser_topic, LaserScan, self.laser_cb_3, queue_size=1)
